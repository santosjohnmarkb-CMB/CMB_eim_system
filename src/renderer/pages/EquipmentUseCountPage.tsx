    ...
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className={`grid grid-cols-1 ${departments.length > 1 ? 'lg:grid-cols-2' : ''} gap-4`}>
        {departments.map((dept) => {
          ...
          const groups = groupByCategoryThenSubcategory(deptCounts, DEPARTMENT_CONFIG[dept].categories)
            .map((group) => ({
              ...group,
              subcategories: group.subcategories.map((sub) => ({
                ...sub,
                items: sub.items.slice().sort((a, b) => b.use_count - a.use_count),
              })),
            }));
                ...
                {groups.length === 0 ? (
                  ...
                  groups.map((group) => (
                    <div key={group.category}>
                      <p className="text-xs font-semibold text-surface-200 mb-2 uppercase tracking-wide">
                        {group.label}
                      </p>
